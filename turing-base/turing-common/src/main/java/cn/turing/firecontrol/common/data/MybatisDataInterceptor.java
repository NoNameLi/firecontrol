...
package cn.turing.firecontrol.common.data;
...
public class MybatisDataInterceptor implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        if(StringUtils.isNotBlank(userId)){
            Boolean isSuperAdmin = userDepartDataService.isSuperAdmin(userId);
//            Boolean isSuperAdmin = "1".equals(userId);
            if(isSuperAdmin){
                return invocation.proceed();
            }
        }*/
        ...
        // 部门数据隔离
        if (depart != null && StringUtils.isNotBlank(BaseContextHandler.getUserID()) && !"1".equals(BaseContextHandler.getIsTenantAdmin())) {
            // 添加用户自己的查询条件
            whereSql.append(" ( 1 = 0 or ");
