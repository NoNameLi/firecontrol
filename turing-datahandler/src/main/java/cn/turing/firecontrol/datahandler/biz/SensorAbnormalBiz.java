...
public class SensorAbnormalBiz {
    ...
    public List<String> queryAlarmTypes(String tenantId){
        ...
        SearchResponse response = transportClient.prepareSearch(ABNORMAL_INDEX).setTypes(ABNORMAL_TYPE_FIREDOOR).setQuery(matchQuery).addAggregation(agg).execute().actionGet();
    ...
    public TableResultResponse queryPage(Integer page, Integer limit, Integer abnormalType, Boolean isHandle,
        ...
        SearchResponse response = transportClient.prepareSearch(ABNORMAL_INDEX).setTypes(ABNORMAL_TYPE_FIREDOOR).setQuery(boolQuery)
                .setFrom(from).setSize(limit).addSort("alarmTime.keyword", SortOrder.DESC).execute().actionGet();
        ...
        return new TableResultResponse(hits.getTotalHits(),abnormals);
    }
    ...
    private Map<String,Object> getCountByfield(String tenantId,String filedName,Date startTime, Date endTime){
        ...
        SearchResponse response = transportClient.prepareSearch(ABNORMAL_INDEX).setTypes(ABNORMAL_TYPE_FIREDOOR).setQuery(boolQuery).addAggregation(agg).execute().actionGet();
