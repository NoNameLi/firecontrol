...
//        if(delta/(60*60) > 0)return delta/(60*60) +"小时前";
    ...
    public static String[] getDatesBetween(Date startDate,Date endDate){
        ...
        while (currentDay.getTime().before(endDate)){
            dates.add(format.format(currentDay.getTime()));
            currentDay.add(Calendar.DAY_OF_MONTH,1);
        }
        dates.add(format.format(endDate));
        String[] results = new String[dates.size()];
        dates.toArray(results);
        return results;
    }
     ...
     * @return
     */
    public static Date getStartBefore(Integer count){
        Calendar calendar = Calendar.getInstance();
