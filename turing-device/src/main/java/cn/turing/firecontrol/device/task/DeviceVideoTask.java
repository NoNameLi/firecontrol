...
public class DeviceVideoTask {
    ...
    private void analisisFire(DeviceVideoExt deviceVideoExt,Constants.AnalysisSolution solution,String alarmType){
        ...
        String analysisDataId = UUIDUtils.generateUuid();
        esTransportUtil.addDocument(solution.getDataEsIndex(),solution.getCode(),analysisDataId,analysisData);
        // 如果本次识别异常：
        if(isAlarm){
            handlerAlarm(sensor,time,alarmType,deviceVideoExt,uploadImageUrl,analysisDataId, Constants.AnalysisSolution.FIRE);
        } else {
            log.info("本次分析结果正常");
