    ...
    public TableResultResponse getAllHardwareFacilitiesList(Query query, Integer channelId) {
        ...
        for (DeviceSensor deviceSensor : list) {
            ...
            if (org.apache.commons.lang.StringUtils.isNotBlank(deviceSensor.getSensorNo())) {
                ...
                SearchSourceBuilder searchSourceBuidler = new SearchSourceBuilder().query(queryBuilder).size(1).sort(sortBuilder);
                SearchResponse searchResponse = esTransportUtil.query(Constant.ESConstant.ES_INDEX_SENSOR, null, searchSourceBuidler);
