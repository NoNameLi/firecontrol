...
import cn.turing.common.entity.turing.WeterMeter;
...
@Slf4j
@Order(96)
public class WaterMeterHandler extends AbstractDeviceEventHandler{
    ...
    public boolean support(DeviceSensorMessage deviceSensorMessage){
        if (LoraDeviceTypeConst.LORA_WATER.equals(deviceSensorMessage.getDeviceType())){
            return true;
        }
        if ("7070".equals(deviceSensorMessage.getDeviceType())){
            return true;
        }

        String data=deviceSensorMessage.getData();
        if (data.startsWith("7070") ){
            return true;
        }
        return  false;
    }
