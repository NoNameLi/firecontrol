 ...
 */
@Data
public class SprayDeviceInfoV2 extends RabbitDeviceInfo {
    ...
    //水压值
    private Double shuiYaValue;
    //电池电压
    private Double dianYaValue;
    //信号值
    private  Integer signal_value;
    public SprayDeviceInfoV2(JSONObject jsonObject) throws Exception {
        super(jsonObject);

        InstructionCode=this.getData().substring(26,28);
        alarmStatus=this.getData().substring(28,30);
        shuiYaValue=Integer.parseInt(this.getData().substring(30,34),16)*0.001;
    ...
    public SensorDetail shuiyaData(){
        SensorDetail sensorDetail=new SensorDetail();
        ...
        sensorDetail.setAlarmValue(ByteUtil.setScaleDouble(shuiYaValue));
