...
@Data
public class NBWellCoverDeviceInfoV1 extends RabbitDeviceInfo {
    ...
    public NBWellCoverDeviceInfoV1(JSONObject jsonObject) throws Exception {
    	super(jsonObject);
        try {
            CMD = this.getData().substring(26, 28);
            if ("01".equalsIgnoreCase(CMD)){
				status = this.getData().substring(28, 30);
				angle=Integer.parseInt(this.getData().substring(30,32),16);
                vol_value = Integer.parseInt(this.getData().substring(32, 36), 16) * 0.001;
                sign_value=Integer.parseInt(this.getData().substring(36,38),16)-110;
