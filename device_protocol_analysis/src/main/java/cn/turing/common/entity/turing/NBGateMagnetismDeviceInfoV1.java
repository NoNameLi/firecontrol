...
@Data
public class NBGateMagnetismDeviceInfoV1 extends RabbitDeviceInfo {
    ...
    public NBGateMagnetismDeviceInfoV1(JSONObject jsonObject) throws Exception {
    	super(jsonObject);
        try {
            CMD = this.getData().substring(26, 28);
            if ("01".equalsIgnoreCase(CMD)){
				left_status = this.getData().substring(28, 30);
				right_status=this.getData().substring(30, 32);
